...
training_loader = DataLoader(
    training_set, batch_size=config.hyperparameters.get("batch_size", 64), shuffle=True, generator=g
)
validation_loader = DataLoader(validation_set, batch_size=config.hyperparameters.get("batch_size", 64), shuffle=False)

# Define model, loss function, and optimizer
model = GarmentClassifier()
loss_fn = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model.parameters(), lr=config.hyperparameters.get("lr", 0.001))
